...
| [upload-image](./skills/upload-image) | Uploading a local image (screenshot, diagram, gif) to a public GitHub bucket repo via the contents API and returning its `raw.githubusercontent.com` URL for embedding in PR descriptions, issues, or markdown (requires push access to the bucket repo) |
